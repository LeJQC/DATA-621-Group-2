...
This study aims to analyze real estate transaction data from New York City to understand the factors influencing property prices. The dataset comprises various features such as location, property type, square footage, and sale price, sourced from public records and real estate listings.

### Exploratory Data Analysis (EDA)

The initial phase involved comprehensive exploratory data analysis to gain insights into the distribution, relationships, and potential patterns within the dataset. EDA techniques included summary statistics, histograms, scatter plots, and correlation analysis. Key findings from EDA informed subsequent steps in data preprocessing and model building.

### Data Preparation

Prior to regression modeling, the dataset underwent preprocessing steps to handle missing values, encode categorical variables, and scale numerical features. Additionally, feature engineering techniques were applied to create new variables or transformations to better capture relationships between predictors and the target variable.

### Regression Model Building Process

Several regression models were considered, including traditional linear regression, robust regression, generalized linear models (GLM), and stepwise regression. Each model type offers unique advantages and assumptions, which were evaluated based on their suitability for the dataset and research objectives. The process involved selecting relevant predictor variables based on domain knowledge and statistical significance, evaluating model performance using cross-validation techniques, and iteratively refining the models.

### Model Selection

Model selection criteria included assessing goodness-of-fit metrics such as R-squared, adjusted R-squared, and mean squared error, as well as diagnostic tests for residual analysis. Both model types were compared based on their ability to generalize to new data and their robustness against outliers and multicollinearity.

In summary, the methodology involved a systematic approach to analyzing real estate transaction data, encompassing exploratory data analysis, data preprocessing, regression model building (including traditional linear regression, robust regression, GLM, and stepwise regression), and model selection. By following these steps, the study aimed to uncover insights into the determinants of property prices in New York City and develop robust predictive models for future property valuation.
